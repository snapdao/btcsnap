...
describe('lib', () => {
  ...
  it('should status as required', async () => {
    ...
    const { utxos, recieveMax, changeMax } = await explorer.getStatus(
      'xpub1',
      true,
    );
  ...
  it('should convert tpub to xpub', () => {
    ...
    const ypub =
      'upub5EKD4VtV4TTbKRrPo7fKUx5i5BP5nP5sb7e3BYi9ydjATnGwKkChYGfDLggrj9PXmxZYtiukGQaHWG5AWVkCmjmbQHviXYy7UMCECw5BEcW';
    const vpub =
      'vpub5ZEjGDr1LN8ensYCDyrNLN56EfxmdSUNZs42iCXeEeu8TL8zUidS4TBoNeMrMPv6vu3vR74GMUM8NxK4Hi5zebzxp25ZyLKbyUPnLNRak2W';
    const key = 'test_key';
    const explorer = new BlockChair(key, BitcoinNetwork.Test);
    const converted = explorer.convertPubKeyFormate(tpub);
    expect(converted).toBe(
      'xpub661MyMwAqRbcFpC47D9RZoghtur8NPtTwgBhTSaJBdaweZ4b66cVLnVvQLbSu9i3MsgxJAXrXMFxS6r8THCUNvYy7SaFsbP513fgxuSRJWn',
    );
    const converted2 = explorer.convertPubKeyFormate(ypub);
    expect(converted2).toBe(
      'ypub6QqdH2c5z79667pmStimqLrwdw2LJGNVe7xPi7Dc9fKzg5qL4KQrmZP3fSRZ8MsRnyZBhwfEycRSmNsyUEpc1MZUgnqxSAtyvMiobVMFL6H',
    );
    const converted3 = explorer.convertPubKeyFormate(vpub);
    expect(converted3).toBe(
      'zpub6jftahH18ngZxukwaV4zYQR49gz8o55Xj24xQJf2iSaq3n7FdEvL8LTSXWqNFs9QpMPXM5cyC8yQ6ADgPfqhLSAg6cBdW2Qiy4C5yJoioxW',
    );
  });
});
