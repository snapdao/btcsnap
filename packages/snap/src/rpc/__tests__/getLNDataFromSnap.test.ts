...
describe('getLNDataFromSnap', () => {
  ...
  it(`should return the password if user use the 'Password' key`, async () => {
    ...
    expect(await getLNDataFromSnap(LNDataToSnap.domain, walletStub, LNDataToSnap.walletId, key, LNDataToSnap.hdPath)).toBe('ln_password_1');
    expect(walletStub.rpcStubs.snap_manageState).toBeCalledTimes(1);
  })

  it(`should return the password if user use the 'Credential' key`, async () => {
    ...
    await getLNDataFromSnap(LNDataToSnap.domain, walletStub, LNDataToSnap.walletId, key, LNDataToSnap.hdPath);
  ...
  it('should throw error if user use the wrong key', async () => {
    ...
    await expect(getLNDataFromSnap(LNDataToSnap.domain, walletStub, LNDataToSnap.walletId, key, LNDataToSnap.hdPath))
      .rejects
