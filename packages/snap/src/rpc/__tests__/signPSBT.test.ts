...
describe('signPsbt', () => {
  ...
  it('should call BtcTx to sign psbt if user approved', async () => {
    ...
    walletStub.rpcStubs.snap_manageState.mockResolvedValue({mfp: fingerprint, network: BitcoinNetwork.Test});
    walletStub.rpcStubs.snap_getBip32Entropy.mockResolvedValue(bip32.fromBase58(testAccountPrivateKey, networks.regtest));
