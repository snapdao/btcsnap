...
describe('signLNInvoice', () => {
  ...
  it('should return signature if user sign the lightning invoice', async () => {
    ...
    const signature = await signLNInvoice(LNDataToSnap.domain, walletStub, LNDataToSnap.invoice, LNDataToSnap.hdPath);
  ...
  it('should reject the sign request and throw error if user reject the sign the lightning invoice', async () => {
    ...
    await expect(signLNInvoice(LNDataToSnap.domain, walletStub, LNDataToSnap.invoice, LNDataToSnap.hdPath))
      .rejects
